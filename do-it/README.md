...
- 버블정렬이나 선택 정렬과 다르게 **오른쪽 -> 왼쪽으로 search** 하며 정렬 한다.
- swap이 아닌 **shift를 통해 정렬**을 진행
- **양손에 카드**를 들고 삽입하며 정렬하는것을 생각하면 쉽게 이해가 가능
  - 오른손에 대상을 들고 왼손에는 정렬한 카드들을 가지고 사이사에 넣는 방식
- 시간 복잡도 : O(N²)
...
- 임의의 위치를 지정 후(피벗) 쪽의 **양쪽 그룹을 나눠가며 정렬** 진행
  - **재귀를 사용하여 구현**
- **Flow** (오름차순 정렬)
  - 1 ) 임의의 위치 지정 - (pivot value selected)
  - 2 ) "1"에서 정한 pivot 값 기준으로 그룹을 나눠 주며(`partition`) 그룹의 기준이 될 index를 반환
      - `while`문을 통해 `left`와 `right` 포인터가 서로 **엇갈릴 때까지**(left <= right) 반복
        - **중복 없이 완벽하게 분리**하기 위함
        - `left < right` 로 할 경우 각 그룹에 중복된 값이 생기기에 무한 재귀가 진행됨
      - `swap`에서 사용하는 조건식 또한  `if(left <= right)`로 진행 해야 함 **이유는 위와 같음** 
    - 2 - 1 ) `A[left]` 와 `pivot` 비교 왼쪽 그룹에는 **작은 값 존재** 하는 것이 목적 문제가 없을 경우 `left++` 진행
    - 2 - 2 ) `A[right]` 와 `pivot` 비교 오른쪽 그룹에는 **큰 값 존재** 하는 것이 목적 문제가 없을 경우 `right--` 진행
    - 2 - 3 ) "2 - 1" ~ "2 - 2" 작업이 종료 후 **각 그룹에 있으면 안되는 값**의 index를 알게되고 해당 값을 서로 `swap` 진행
...
- 배열의 데이터 분할 후 분할된 **집합을 합치며** 정렬
  ...
  - 병합 시 그룹의 첫번째 `index` 포인터를 둔 후 **포인터를 옮기는 방식**으로 진행
    - 각 그룹의 시작 index를 기준으로 포인터를 이동
- **Flow** 
  - 1 ) 배열의 중앙 인덱스 값을 구한다.
  - 2 ) 재귀를 통해 가장 작은 수준의 그룹까지 분활 한다.
  - 3 ) 가장 작은 수준의 그룹을 병합해 가며 정렬으 진행한다.
    - 3 - 1 ) 정렬 값을 저장할 임시 배열 생성 ( `right - left + 1`크기로 필요한 사이즈의 임시 배열 사이즈를 구할 수 있음 )
      - 임시 배열의 위치를 포인터할 변수 선언
    - 3 - 2 ) 왼쪽 그룹의 시작 index 와 오른쪽 그룹의 시작 인덱스를 지정한다.
      - 왼쪽 : `index = left`
      - 오른쪽 : `index = mid + 1`
    - 3 - 3 ) 왼쪽 그룹과 오른쪽 그룹이 모두 범위 내에 있을 때 서로 바교하며 임시 배열에 정렬 값 저장
      - `while`을 사용하여 정렬 진행
    - 3 - 4 ) 비교가 불가능했던 남은 그룹들의 값을 임시 배열에 저장
...
- 전체 선택지 중 지금 고르는 선택지를 **최선이라 가정하는 알고리즘**
  - 일반적인 상황에서는 **최적의 해를 보장하지 않는다**  
  - ✅ 단, 그리디를 사용할 수 있는 문제는 **항상 "탐욕적 선택이 항상 최적해를 보장하는" 특정한 조건(규칙)이 반드시 숨어 있다.**
- 그리디 알고리즘 **사용 여부 포인트** (문제의 특성)
  - **명확한 수학적 포함 관계**: 큰 단위가 항상 작은 단위의 배수일 때 (ex. 정규 동전 체계)
  - **선택의 독립성** (나비효과 없음): 현재의 최선의 선택(가장 빨리 끝나는 것, 가장 가벼운 것)이 **미래의 선택지에 악영향을 주지 않을 때**
  - **명확한 정렬 기준**: 시간, 무게, 가성비 등 단일 기준으로 오름차순/내림차순 정렬했을 때 논리적 모순이 발생하지 않을 때
- **핵심 이론**
  - 1 . 해 선택 : 현 상태에서 **가장 최선이라 생각되는 해**를 선택 (가장 큰 것, 가장 빠른 것 등)
...
- **에라토스테네스의 체**를 사용하여 **범위 내 소수**를 찾는것이 일반적이다.
  ...
  - **원리**
    - 찾고자 하는 수의 범위 배열 생성
    - "2"부터 시작 하여 탐색 진행
    - 현재 확인중인 숫자가 **남아있는 숫자**라면 해당 숫자의 **배수에 해당 하는 수 제거**
    - 마지막까지 **배열에 남아 있는 모든 수가 소수**이다.
  - **핵심**
    - **소수의 배수**는 **합성수**(소수가 아닌 수)라는 점
    - 반복의 범위는 대상의 **마지막수의 제곱근**까지만 진행
      - 합성수는 반드시 **제곱근 이하의 약수를 가지**므로 그 이상의 수는 합성수이기에 **불필요한 반복 범위**를 배제 할 수 있다.
...
```java
public class Main{
  private static boolean[] getPrimeArr(int num) {
      boolean[] isNotPrime = new boolean[num + 1];
      // 0, 1 은 소수가 될 수 없음
      isNotPrime[0] = true;
      isNotPrime[1] = true;
  
      // 최적화: 제곱근은 반복문 밖에서 딱 한 번만 계산 진행
      ...
      for (int i = 2; i <= limit; i++) {
          // 아직 지워지지 않은 수 인지 체크
          if (!isNotPrime[i]) {
              // i의 배수들을 징검다리 점프(+= i)하며 지우기!
              // ex) i가 3일 경우 - 9, 12, 15, 16 ...
...
public class Main{
  public static boolean isPrime(int num){
    // 1. 기본 처리: 0과 1은 소수가 될 수 없다.
    if (num <= 1) return false;

    // 2. "2"는 유일한 짝수 소수이다.
    if (num == 2) return true;

    // 3. "2"를 제외한 짝수는 소수가 될 수 없다.
    if (num % 2 == 0) return false;

    // ✅ 약수 확인 범위 로직 (소수는 1과 자기 자신만 약수여야 함)
    // - 약수는 항상 [작은 수 * 큰 수]로 쌍(Pair)을 이룹니다.
    // - 따라서 '반환점(제곱근)'까지만 검사하면, 그 뒤는 검사할 필요가 없습니다.
    // 🔍  Math.sqrt(num); 를 반복문 내에서 작업하면 매번 계산하기에 반복문 밖에서 계산은 필수이다.
    // [예시: num = 36일 때]
    // 1 * 36
    // 2 * 18
    // 3 * 12
    ...
    // - "i = 3"시작: 위에서 짝수를 다 제외했으므로 홀수만 확인합니다.
    // "i += 2" : 이미 상단 조건식에서 짝수는 제거 했기에 홀수(3, 5, 7...)만 검사가 가능하다.
...
## 18 ) 오일러의 피 (Euler's Phi)
> **정의** : 1부터 "N"까지의 범위에서, "N"과 "서로소"인 자연수의 개수를 구하는 알고리즘  
> **서로소?** : 두개의 수가 주어질 떼 "1" 외에는 **공통된 약수**(최대공약수)가 **없는 관계**  
> **약수?** : "10"의 약수 [1, 2, 5, 10]  
> - [⭕] 예: 7과 10은 공통된 약수가 "[1]" **한개 뿐**이므로 **서로소**이다.    
> - [❌] 예: 8과 12는 공통된 약수가 "[1, 2, 4]" **총 3개** 이므로 **서로소가 아니다.**
- **GCD(n, k)**
  - `n`과 `k`의 **최대공약수**를 구하는 것
  - ex) `GCD(8, 12)` : 8과 12의 **최대공약수**를 구해라
    - `GCD(n, k) = 1` : n과 k의 최대 공약수가 1(서로소)인 것을 구하라 
- **알고리즘에 자주 출제 되지는 않으나 원리를 알아야 접근이 가능한 문제임**

- 참고 문제
  - GCD(n, k) = 1 : [참고](https://github.com/edel1212/algorithm/blob/main/src/do_it/quiz/revange/GCD_R.java)
...
- **구조**:노드 중심으로 2차원 배열로 그래프를 표현함 - **체스판 형태 (`A[N][N]`)**
- **데이터 저장**:
  - 가중치가 없을 경우: 연결 여부를 `1(연결), 0(미연결)`으로 표시
  - 가중치가 있을 경우: `0, 1 `대신 해당 칸에 **가중치(비용)** 를 직접 넣음
- **주의 사항** :
  - 노드에 관련되어 있는 엣지를 탐색하려면 N번 탐색해야 하므로, **노드의 개수보다 엣지가 적을 때 공간 효율성이 떨어짐**
  - 노드의 값이 클 경우 2차원 배열을 선언 자체를 할 수도 없을 수 있다.
      - ex) `N = 10^9` 경우
  - ✅ 인접 행렬은 노드 개수에 따라 **사용 여부**를 적절하게 **판단하는 능력이 필요**
-  **주요 사용 알고리즘** : 플로이드-워셜 (Floyd-Warshall), 빠른 연결 확인 문제

#### 인접 리스트 (Adjacency List)

![img_4.png](img_4.png)

- **정의**: 그래프 알고리즘에서 가장 많이, 가장 기본적으로 사용되는 핵심 자료 구조.
- **구현 방법**: 노드 개수만큼 가변적인 `ArrayList`를 선언하여 '개인 주소록'을 쥐여주는 방식
    - 기본 구조: `List<List<Integer>>`
    - 실전 최적화 구조: `ArrayList<Integer>[]` (리스트를 담는 배열. 속도와 메모리 효율이 가장 좋음)
- **데이터 저장 방법**:
  - 가중치가 없을 경우: 주소록에 '도착 노드' 번호만 추가. (예: `graph[1].add(2)`)
  - 가중치가 있을 경우: 도착 노드와 가중치를 묶은 객체(`Class`)를 만들어 리스트에 넣음 
- **특징**:
  - 연결된 선(간선)의 개수만큼만 메모리를 차지, 노드 개수(N)가 10만 개 이상으로 커도 메모리 터질 일이 절대 없음
  - 특정 노드와 **연결된 엣지**만 조회 하므로, 낭비되는 탐색 시간이 없음
...
> 2. 여러 노드들을 그룹(집합)으로 묶고, 같은 그룹인지 판별
### **핵심 구조** : 1차원 배열(`parent`)을 사용하여 각 노드의 **'대표 노드'** 를 기록

1. `find연산` (대표 찾기 & 경로 압축) :
- **목적**: 특정 노드가 속한 집합의 **'최고 대표 노드'**를 찾는 연산.
- **동작**: 내 부모가 나 자신(`parent[x] == x`)일 때까지 **재귀**를 통해 끝까지 거슬러 올라감.
- **경로 압축** : 대표를 찾고 `return` 시, 거쳐간 모든 노드의 부모를 **'최고 대표 노드'**로 업데이트
  - ex) `return parent[x] = find( parent[x] );`

2. `union연산` (두 집합 합치기)
- **목적**: 두 개의 다른 노드(또는 집합)를 **하나의 같은 집합으로 연결**
  - 무작정 연결하는 것이 아닌, `find(a)`와 `find(b)`를 통해 각각의 **'대표 노드'** 를 찾음
  - 두 대표가 다르다면(다른 집합이라면), **한쪽 대표를 다른 쪽 대표 밑으로 종속 시킴**
    - ex ) `parent[rootB] = rootA;`

### **Flow**
- 대표 노드를 저장 할 `parent[]` 선언 후 **index 와 value 를 같게** 초기화
- `union연산`을 통해 각 노드(또는 집합)을 하나로 합침
  - 전달 받은 a, b의 대표 값을 찾은 후 값이 `rootA, rootB`가 다를 경우 하나로 합침
- `find연산`을 통해 해당 노드의 대표 값을 찾고, 경로를 압축 시킴
  - 1 . 대상 노드의 `parent[]`의 `value == index` 동일 값인지 확인
  - 2 . 같지 않다면 **재귀를 통해** root node를 찾음
  - 3 . 대표 노드에 도달 시 재귀 함수를 빠져나가며 **거쳐간 노드들의 value 값을 루트 노드 값으로 변경**
    - 경로 압룩을 통해 find 연산의 속도가 `O(1)`으로 변경됨

```java
class Main{
    // 사이클이 발생하면 false, 정상적으로 합쳐지면 true를 반환하는 union 함수
    public static boolean union(int a, int b) {
      int rootA = find(a);
      int rootB = find(b);
  
      // 🚨 핵심: 두 노드의 최고 대표가 이미 같다면? (이미 같은 집합이라면?)
      if (rootA == rootB) {
        // 사이클 발생
        return false; 
      } // if
  
      // 대표가 다르면 정상적으로 한쪽을 다른 쪽에 종속시킴
      parent[rootB] = rootA;
      return true;
    }

    private static int find(int x){
        // 내가 Root 일 경우 내 자신 반환
        if(parent[x] == x) return x;

        // [재귀] 핵심 로직 - 경로 압축 루트를 찾으며 값을 바꿈
        return parent[x] = find( parent[x] );
    }
}
```
- 참고 문제
  - 집합의 표현 : [참고](https://github.com/edel1212/algorithm/blob/main/src/do_it/quiz/revange/%EC%A7%91%ED%95%A9%EC%9D%98_%ED%91%9C%ED%98%84_R2.java)
  - 여행 가자 : [참고](https://github.com/edel1212/algorithm/blob/main/src/do_it/quiz/revange/%EC%97%AC%ED%96%89_%EA%B0%80%EC%9E%90_R3.java)
  - 거짓말 : [참고](https://github.com/edel1212/algorithm/blob/main/src/do_it/quiz/revange/%EA%B1%B0%EC%A7%93%EB%A7%90_R.java)


## 21 ) 위상 정렬
### 핵심 목적
> **사이클이 없는** 방향 그래프에서 **노드 순서를 찾는** 알고리즘  
### **핵심 구조** : **"진입차수(`in-degree`)"를 활용해** 정렬하는 알고리즘
- 진입차수? : 나를 향하는 **화살표(노드)의 개수**를 저장한 배열
  - `inDegree[endNode]++`
- **중요 포인트** : 
  - 정렬 관계 결과가 항상 같지 않음
    - ex) 롤에서 코어템을 만들 때, 하위템 A와 B를 사는 순서는 서로 바뀌어도 상관없는 것과 같음.
  - `A->B` 와 같이 **선후관계(선수 조건)가 있음**
    - ex) 스타크래프트에서 **배럭을 지어야 팩토리를 지을 수 있다.**
  - **진입 차수가 0인** 노드만 큐(Queue)에 **들어갈 수 있다.**

### **Flow**
- 진입차수를 저장 할 `inDegree[]` 선언
- 인접 행렬 생성 후 초기화 시 진입 차수의 값 또한 초가화 진행 `inDegree[end]++`
- `Qeueue` 선언 후  진입차수의 **value가 0**인 노드만 **큐에 저장**
  - 1 . `Qeueue`를 순회하며 `queue.poll()`로 나온 노드를 정렬 순위를 높게 지정
    - ✅ 0인 노드가 여러개가 생길 수 있기에, 위상정렬이 늘 같은 결과를 보장하지 하는 이유
  - 2 . BFS와 같은 구조인 `for( int next : graph.get(c) )`에서 다음 노드의 진입 차수 값을 감소 `inDegree[next]--`
  - 3 .  반복문 내 **진입 차수가 0인** 노드만 큐(Queue)에 삽입
...
## 22 ) 다익스트라 (최단거리 알고리즘)
### 핵심 목적
> **시작 위치가 정해지고**, **음수 간선이 없는 상태**에서 그래프에서 최단거리를 구하는 알고리즘
- '그리디(Greedy)' 알고리즘이며, "지금 이 순간 가장 가까운 노드가 최단 거리로 판단하는 알고리즘"
### **핵심 구조** : **"거리 배열(`distance`)"를 활용해** 최단거리를 구함
- **기능** : **출발 노드**와 **모든 노드간**의 **최단 거리** 탐색
  - 문제 요구 사항이 **최단 거리를 구하는 문제**일 경우 해당 알고리즘을 사용하여 해결 가능
