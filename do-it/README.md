...
- **Flow** (오름차순 정렬)
  ...
  - 2 ) "1"에서 정한 pivot 값 기준으로 그룹을 나눠 주며 그룹의 기준이 될 index를 반환
    - 2 - 1 ) `A[left]` 와 `pivot` 값을 비교하여, 왼쪽 그룹에는 작은 값만 추출 큰 값이 있을 땐 `left++` 진행
    - 2 - 2 ) `A[right]` 와 `pivot` 값을 비교하여, 오른쪽 그룹에는 큰 값만 추출 작은 값이 있을 땐 `right--` 진행
